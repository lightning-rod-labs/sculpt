...
import copy
import json
from .utils import load_config, DEFAULT_SYSTEM_PROMPT


class _BaseSculptor:
    """
    Contains all common logic for both synchronous and asynchronous Sculptor classes.
    ...
    def add(
        ...
        if field_type_str == "array":
            ...
            if enum and items is None:
                ...
                if items is None:
                    raise ValueError(
                        "For 'array' type, you must provide 'items' or supply 'enum' for an enumerated array."
                    )
    ...
    @classmethod
    def from_config(cls, filepath: str, **kwargs: Any) -> "_BaseSculptor":
        """
        Creates an instance from a config file (JSON or YAML).
        Note: The client-specific parameters are not handled here.
        """
    ...
    def _build_schema_for_llm(self) -> Dict[str, Any]:
        ...
        top_properties = {
            field_name: build_subschema(field_meta)
    ...
    def _parse_response(self, resp: Any, data: Dict[str, Any], merge_input: bool) -> Dict[str, Any]:
        """Parses the LLM response and optionally merges it with the input data."""
    ...
    def _build_request_params(self, data: Dict[str, Any], attempt: int) -> Dict[str, Any]:
        """Builds the parameters for the API call."""
        schema_for_llm = self._build_schema_for_llm()
        # The client's base_url is used to determine the response_format.
        base_url = self.get_client_base_url()
        return {
            "model": self.model,
            "messages": [
            ...
            "response_format": (
                ...
                if ("deepseek" in str(base_url).lower() or "deepseek" in str(self.model).lower())
    ...
    def get_client_base_url(self) -> str:
        """
        Abstract method. Subclasses must implement this to return
        the base URL from their client.
        """
        raise NotImplementedError("Subclasses must implement get_client_base_url().")
